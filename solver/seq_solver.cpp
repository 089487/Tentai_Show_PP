// Modern C++23 refactor with classes and STL
#include <algorithm>
#include <cctype>
#include <cstdint>
#include <cstdio>
...
public:
    ...
    // Linear scan helper (kept simple to preserve behavior)
...
public:
    ...
    std::optional<State> solve() {
        ...
        // Pre-fill tiles that have dots in their center (odd, odd)
        ...
        while (!q.empty()) {
            ...
            for (int d = 0; d < static_cast<int>(p_.dotCount()); ++d) {
                ...
                // Expand from neighbors of existing tiles
                ...
                // Tiles touching the dot (also considered even if the dot already has tiles)
...
private:
    const Puzzle &p_;
    std::vector<std::vector<std::uint64_t>> zobrist_; // [w*h][numDots]
