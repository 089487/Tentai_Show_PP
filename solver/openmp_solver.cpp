    ...
    std::optional<State> solve() {
        ...
        while (!q.empty() && !solution_found) {
            ...
            for (int i = 0; i < level_size; ++i) {
                current_level.push_back(std::move(q.front()));
                q.pop();
            }

            #pragma omp parallel
            {
                std::vector<State> local_q;

                #pragma omp for schedule(dynamic)
                for (int i = 0; i < level_size; ++i) {
                    if (solution_found) continue;

                    const State& s = current_level[i];
                                ...
                                if (s.at(x, y) == d) {
                                    ...
                                    for (auto& dir : dirs) {
                                        ...
                                        if (isValidTile(s, sx, sy)
                                            ...
                                            if (nx != sx || ny != sy) {
                                                if (next.at(sx, sy) == -1) {
                                                    next.at(sx, sy) = static_cast<int16_t>(d);
                                            ...
                                            local_q.push_back(std::move(next));
                                        }
                                    }
                                }
                            }
                ...
                #pragma omp critical
                {
                    for (auto& next_state : local_q) {
                        if (visited_.insert(next_state.hash).second) {
