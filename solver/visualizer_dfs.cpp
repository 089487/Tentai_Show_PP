/**
 * @file visualizer_dfs.cpp
 * @brief DFS-based solver for Tentai Show with visualization hooks.
 *
 * This program solves the puzzle using a backtracking DFS and can save the
 * history of states (including backtracking) to a file for visualization.
 */
...
class Solver {
   public:
    ...
    void setHistoryFile(const std::string& filename) {
        history_file_.open(filename);
        if (history_file_) {
            history_file_ << p_.width() << " " << p_.height() << "\n";
            history_file_ << p_.dotCount() << "\n";
            for (const auto& dot : p_.dots()) {
                history_file_ << dot.x() << " " << dot.y() << " "
                              << dot.is_black() << "\n";
            }
        }
    }
   ...
   private:
    ...
    bool dfs() {
        if (filled_count_ == w_ * h_) {
            bool success = allDotsUsed();
            if(success) writeHistory();
            return success;
        }
                    ...
                    for (auto& dir : dirs) {
                        ...
                        if (inTileBounds(nx, ny) && grid_[ny][nx] != -1) {
                            ...
                            if (isValidTile(sx, sy) || (sx == x && sy == y)) {
                                bool sym_empty = inTileBounds(sx, sy) && (grid_[sy][sx] == -1);
                                if (!inTileBounds(sx, sy) || (grid_[sy][sx] == d) || sym_empty) {
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        std::fprintf(stderr, "Usage: %s <input_file> [--history <history_file>]\n", argv[0]);
        return 1;
    }
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--history") {
            if (i + 1 < argc) {
                ...
                std::fprintf(stderr, "--history option requires one argument.\n");
                return 1;
            }
        } else {
            input_file = arg;
    ...
    if (!history_file.empty()) {
        solver.setHistoryFile(history_file);
    }
