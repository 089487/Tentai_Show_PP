/**
 * @file visualizer.cpp
 * @brief Sequential solver for the Tentai Show (Symmetry) puzzle with visualization hooks.
 *
 * This program solves the puzzle and can optionally save the history of states
 * explored during the search to a file. This history can then be used by an
 * external script to generate a visualization of the search process.
 */
...
class Solver {
   ...
   private:
    ...
    void writeHistory(const State& s) {
        if (history_file_) {
            for (int y = 0; y < p_.height(); ++y) {
                for (int x = 0; x < p_.width(); ++x) {
                    history_file_ << s.at(x, y) << " ";
                }
            }
            history_file_ << "\n";
        }
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        std::fprintf(stderr,
                     "Usage: %s <input_file> [--history <history_file>]\n",
                     argv[0]);
        return 1;
    }
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--history") {
            if (i + 1 < argc) {
                ...
                std::fprintf(stderr,
                             "--history option requires one argument.\n");
                return 1;
            }
        } else {
            input_file = arg;
